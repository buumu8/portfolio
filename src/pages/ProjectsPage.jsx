...
function ProjectsPage() {
  ...
  const fallbackImages = Array.from({ length: 10 }, (_, i) => `${import.meta.env.BASE_URL}/images/fallback_${i + 1}.jpg`);
  ...
  useEffect(() => {
    ...
    const projectData = [
      {
        id: "game_ai_fuzzy_race_track",
        name: "Game AI Fuzzy Race Track",
        description:
          "Implemented a fuzzy logic-based AI to navigate a race track in a game, demonstrating advanced AI techniques and real-time decision-making.",
        image: `${import.meta.env.BASE_URL}/images/racetrack.png`,
        tags: ["Game AI", "AI", "Fuzzy Logic"],
        href: "https://youtu.be/GJXkzKv-CUk",
      },
      // {
    ...
    const withImages = projectData.map((proj) => ({
      ...proj,
      image: proj.image || fallbackImages[hashStringToIndex(proj.id, fallbackImages.length)],
    }));
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentProjects.map((project) => (
              <a href={project.href} target="_blank">
                <div key={project.id} className="border rounded-md shadow-sm hover:shadow-md transition bg-gray-50 overflow-hidden flex flex-col">
                  {/* Project Image */}
                  {project.image && (
